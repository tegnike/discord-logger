...
test('failed or private Discord permission removes its source and destination',()=>{
 const p=deriveMemoryPolicy(contract,new Map(),0);
...
test('approved readable channels receive a bounded lease',()=>{
 const p=deriveMemoryPolicy(contract,new Map([['2','public']]),0);
...
test('restricted channels are processed but excluded from public runtime routes',()=>{
 const p=deriveMemoryPolicy(contract,new Map([['2','restricted']]),0);
 assert.equal(p.scopes['discord:channel:2'].audience,'restricted');
 assert.deepEqual(p.runtimes['discord-public'].destinations,{});
 assert.deepEqual(p.runtimes['x-public'].destinations['twitter:public'],['twitter:public']);
});
